...
namespace Fractural.Tasks
{
    public static partial class GDTaskExtensions
    {
        ...
        {
            GDTaskCompletionSource<T> promise = new GDTaskCompletionSource<T>();

            task.ContinueWith((x, state) =>
            {
                GDTaskCompletionSource<T> p = (GDTaskCompletionSource<T>)state;
        ...
        {
            GDTaskCompletionSource promise = new GDTaskCompletionSource();

            task.ContinueWith((x, state) =>
            {
                GDTaskCompletionSource p = (GDTaskCompletionSource)state;
        ...
        public static Task<T> AsTask<T>(this GDTask<T> task)
        {
            try
            {
                ...
                {
                    using (StateTuple<TaskCompletionSource<T>, GDTask<T>.Awaiter> tuple = (StateTuple<TaskCompletionSource<T>, GDTask<T>.Awaiter>)state)
                    {
                        (TaskCompletionSource<T> inTcs, GDTask<T>.Awaiter inAwaiter) = tuple;
                        try
                        {
        ...
        public static Task AsTask(this GDTask task)
        {
            try
            {
                ...
                awaiter.SourceOnCompleted(state =>
                {
                    using (StateTuple<TaskCompletionSource<object>, GDTask.Awaiter> tuple = (StateTuple<TaskCompletionSource<object>, GDTask.Awaiter>)state)
                    {
                        (TaskCompletionSource<object> inTcs, GDTask.Awaiter inAwaiter) = tuple;
                        try
                        {
                            inAwaiter.GetResult();
        ...
        public static async GDTask Timeout(this GDTask task, TimeSpan timeout, DelayType delayType = DelayType.DeltaTime, PlayerLoopTiming timeoutCheckTiming = PlayerLoopTiming.Process, CancellationTokenSource taskCancellationTokenSource = null)
        {
            ...
            GDTask<bool> timeoutTask = GDTask.Delay(timeout, delayType, timeoutCheckTiming, delayCancellationTokenSource.Token).SuppressCancellationThrow();
        ...
        public static void Forget(this GDTask task)
        {
            ...
            else
            {
                awaiter.SourceOnCompleted(state =>
                {
                    using (StateTuple<GDTask.Awaiter> t = (StateTuple<GDTask.Awaiter>)state)
                    {
                        try
                        {
                            t.Item1.GetResult();
        ...
        public static void Forget<T>(this GDTask<T> task)
        {
            ...
            else
            {
                awaiter.SourceOnCompleted(state =>
                {
                    using (StateTuple<GDTask<T>.Awaiter> t = (StateTuple<GDTask<T>.Awaiter>)state)
                    {
                        try
                        {
                            t.Item1.GetResult();
