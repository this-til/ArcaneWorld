...
namespace Fractural.Tasks
{
    public partial struct GDTask
    {
        ...
        {
            using (ArrayPoolUtil.RentArray<GDTask<T>> span = ArrayPoolUtil.Materialize(tasks))
            {
        ...
        {
            using (ArrayPoolUtil.RentArray<GDTask> span = ArrayPoolUtil.Materialize(tasks))
            {
            ...
            public WhenAnyLRPromise(GDTask<T> leftTask, GDTask rightTask)
            {
...

                {
                    ...
                    else
                    {
                        ...
                        {
                            using (StateTuple<WhenAnyLRPromise<T>, GDTask<T>.Awaiter> t = (StateTuple<WhenAnyLRPromise<T>, GDTask<T>.Awaiter>)state)
                            {
                ...
                RIGHT:
                {
                    ...
                    else
                    {
                        awaiter.SourceOnCompleted(state =>
                        {
                            using (StateTuple<WhenAnyLRPromise<T>, Awaiter> t = (StateTuple<WhenAnyLRPromise<T>, GDTask.Awaiter>)state)
                            {
            ...
            public WhenAnyPromise(GDTask<T>[] tasks, int tasksLength)
            {
                ...
                for (int i = 0; i < tasksLength; i++)
                {
                    ...
                    else
                    {
                        awaiter.SourceOnCompleted(state =>
                        {
                            using (StateTuple<WhenAnyPromise<T>, GDTask<T>.Awaiter, int> t = (StateTuple<WhenAnyPromise<T>, GDTask<T>.Awaiter, int>)state)
                            {
            ...
            public WhenAnyPromise(GDTask[] tasks, int tasksLength)
            {
                ...
                for (int i = 0; i < tasksLength; i++)
                {
                    ...
                    else
                    {
                        awaiter.SourceOnCompleted(state =>
                        {
                            using (StateTuple<WhenAnyPromise, Awaiter, int> t = (StateTuple<WhenAnyPromise, GDTask.Awaiter, int>)state)
                            {
                                TryInvokeContinuation(t.Item1, t.Item2, t.Item3);
                            }
                        }, StateTuple.Create(this, awaiter, i));
