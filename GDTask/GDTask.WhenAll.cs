            ...
            public WhenAllPromise(GDTask<T>[] tasks, int tasksLength)
            {
                ...
                for (int i = 0; i < tasksLength; i++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            using (StateTuple<WhenAllPromise<T>, GDTask<T>.Awaiter, int> t = (StateTuple<WhenAllPromise<T>, GDTask<T>.Awaiter, int>)state)
                            {
            ...
            public WhenAllPromise(GDTask[] tasks, int tasksLength)
            {
                ...
                for (int i = 0; i < tasksLength; i++)
                {
                    ...
                    else
                    {
                        awaiter.SourceOnCompleted(state =>
                        {
                            using (StateTuple<WhenAllPromise, Awaiter> t = (StateTuple<WhenAllPromise, GDTask.Awaiter>)state)
                            {
